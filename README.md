...
To support 10,000 users the architecture would change substantially. The single Express process would be horizontally scaled behind a load balancer, with stateless app servers. localStorage would be replaced by a managed database (e.g. PostgreSQL via Supabase) so receipts sync across devices. Receipt images would move to object storage (S3 / Supabase Storage) instead of being embedded in localStorage. Authentication would be added so each user only sees their own records. Rate limiting (per-user and global) would protect the OpenAI budget and prevent abuse. AI extraction — which is slow and costly — would move to a background job queue (e.g. BullMQ) so the HTTP request returns immediately and the user polls or receives a websocket update. Monitoring and structured logging (Sentry, Datadog) would track API errors, latency, and cost. AI cost controls — per-user daily limits, image downscaling before upload, and model tier selection — would keep the OpenAI bill predictable.
